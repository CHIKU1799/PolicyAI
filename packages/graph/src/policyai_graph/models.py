...
class Node(Base):
    ...
    # --- Bitemporal axes ---------------------------------------------------
    # valid-time: when the regulation/entity is in force in the real world.
    effective_from: Mapped[date | None] = mapped_column(Date, nullable=True)
    effective_to: Mapped[date | None] = mapped_column(Date, nullable=True)
    # transaction-time: when PolicyAI learned / retired it (recorded_at ~ created_at).
    recorded_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
    invalidated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    superseded_by_node_id: Mapped[UUID | None] = mapped_column(
...
class Edge(Base):
    ...
    # Relationship validity (e.g. an APPLIES_TO that stops being true when a reg is superseded).
...
class RawDocument(Base):
    ...
    # Full-text index for hybrid retrieval (vector + keyword). Generated by Postgres,
    # never written by the ORM; a GIN index on it backs websearch_to_tsquery lookups.
